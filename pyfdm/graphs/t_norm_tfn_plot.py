...
def t_norm_tfn_plot(data, title=None, ax=None):
    """
    ...
    # Plot T-norm (Min) result
    y_values = np.array(y_values)
    y_values_t_norm = np.min(y_values, axis=0)
    ax.plot(x_values, y_values_t_norm, label='T-norm (Min)', linestyle='--', linewidth=4, color='red')
